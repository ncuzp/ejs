 ...
 * @return {String}
 */
// 获取文件相对路径, 详见Node官网参考path.resolve
 ...
 * @static
 */
// 纪录执行过程中的错误, 抛出到外层调用则, 指明了具体的行号及出错信息, 如下的出错字样:
// { [ReferenceError: ./src.ejs:3
//     1| hello ,name><% name %>
//     2| hello ,locals.name><% locals.name %>
//  >> 3| <%= name2.d %>
//     4| <%# 这是注释 %>
//     5| <%% name %>
//     6|

// name2 is not defined] path: './src.ejs' }
 ...
 * @public
 */
// 主要是为了兼容以前的scope配置项, 如果抛弃该选项此处可直接调用Template实例的compile方法
 ...
 * @public
 */
// 对外暴露渲染方式之二, 与 `render`不同此处主要传入文件路径然后读取文件内容
...
, compile: function () {
    ...
    var returnedFn = function (data) {
      var include = function (path, includeData) {
        // 文件中包含`include`操作, 例如模板中包含`<% include index %>`此处实现递归处理
